...
int Protocol1PacketHandler::ping(PortHandler *port, uint8_t id, uint16_t *model_number, uint8_t *error)
{
  ...
  if (result == COMM_SUCCESS && model_number != 0)
  {
    ...
    uint8_t data_read_byte = 0; // Variable para guardar el único byte

    // 1. Forzamos la lectura a 1 SOLO BYTE (len = 1)
    //    (Nota: He añadido el puntero 'error' a la llamada, que faltaba en tu código original)
    result = readTxRx(port, id, 0, 1, &data_read_byte, error);  
    
    if (result == COMM_SUCCESS)
    {
      // Convertimos el byte leído a un número de 16 bits
      uint16_t model_val = (uint16_t)data_read_byte;

      // 2. Lógica de reemplazo: si es 3, cámbialo a 29
      if (model_val == 3)
      {
        model_val = 29;
      }

      // Asignamos el valor final (sea el original o 29)
